...
async function renderAlbum(container, albumId) {
    ...
    if (!album) {
        container.replaceChildren(make('p', 'text-secondary mt-4', 'Album non trovato.'));
        return;
    }
...
async function renderPlaylist(container, playlistId) {
    ...
    if (!vp) {
        container.replaceChildren(make('p', 'text-secondary mt-4', 'Playlist non trovata.'));
        return;
    }
...
function renderUserPlaylist(container, playlistId) {
    ...
    const meta = make('div', 'playlist-meta');
    append(meta, make('strong', '', state.displayName), ` • ${playlist.tracks.length} brani`);
...
function renderProfile(container) {
    ...
    if (state.profilePhoto) {
        ...
        avatarEl = make('div', 'playlist-cover-large');
        avatarEl.style.cssText = 'border-radius:50%; background:linear-gradient(135deg,#1db954,#191414); display:flex; align-items:center; justify-content:center;';
...
function makePlaylistHeader(coverSrc, type, title, ...metaNodes) {
    ...
    if (coverSrc) {
        ...
        coverEl.style.cssText = 'background:linear-gradient(135deg,#450af5,#c4efd9); display:flex; align-items:center; justify-content:center;';
...
function renderTrackList(trackList, showAlbumCol = true) {
    ...
    list.append(header);
    trackList.forEach((track, index) => list.append(makeTrackRow(track, index, ids, showAlbumCol)));
    return list;
}
