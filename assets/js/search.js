...
async function renderSearch(container) {
  ...
  GENRES.forEach((g) => {
    ...
    card.addEventListener("click", () =>
      navigateTo("genre-" + encodeURIComponent(g.name)),
    );
...
async function performSearch(query) {
  ...
  append(
    loading,
    make("div", "spinner-border spinner-border-sm text-success me-2"),
    "Ricerca in corso...",
  );
  ...
  try {
    ...
    if (
      ...
      tracks.unshift(normalizeTrack(CUSTOM_TRACK_DARIO));
    }
    ...
    results.replaceChildren(
      make("p", "text-secondary mt-3", "Errore nella ricerca. Riprova."),
    );
  }
}
