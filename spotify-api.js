...
function logoutSpotify() {
    ['spotify_access_token', 'spotify_token_expiry',
     'spotify_refresh_token', 'spotify_code_verifier'].forEach(k => localStorage.removeItem(k));
}
...
async function apiGetFeaturedPlaylists() {
    const data = await spotifyFetch('/browse/featured-playlists?limit=8&country=IT&locale=it_IT');
...
async function apiGetCategories() {
    const data = await spotifyFetch('/browse/categories?limit=20&country=IT&locale=it_IT');
    return data.categories.items;
}

async function apiSearch(query) {
    const params = new URLSearchParams({ q: query, type: 'track,album', limit: 20, market: 'IT' });
...
async function apiGetAlbumTracks(id, albumData) {
    const data = await spotifyFetch('/albums/' + id + '/tracks?limit=50&market=IT');
...
async function apiGetPlaylistTracks(id) {
    ...
    const data = await spotifyFetch('/playlists/' + id + '/tracks?limit=50&market=IT&fields=' + fields);
...
function normalizeTrack(track, albumOverride) {
    ...
    const cover = images[0] ? images[0].url : createCover('?', '#1db954', '#191414');
    return {
        id: track.id,
        title: track.name,
...
function normalizeAlbum(album) {
    ...
    return {
        ...
        cover: album.images && album.images[0] ? album.images[0].url : createCover('?', '#1db954', '#191414'),
    };
}
...
function normalizePlaylist(playlist) {
    ...
    return {
        ...
        cover: playlist.images && playlist.images[0] ? playlist.images[0].url : createCover('?', '#1db954', '#191414'),
    };
}